var obtainParentThroughClassName = function(el, parentSelector,classValue){
    ...
    var classNameValue;
    var elementOfClass;
    console.log(elementOfClass);
    while (p !== parentSelector) {
        o = p;  
        classNameValue=p.className;
        if(classValue===classNameValue){
            elementOfClass=p;
            break;
        }
        p = o.parentNode;
...
var obtainElementParents = function(el, parentSelector ) {
...
var createFragment = function(count) {
    ...
    for (var i = 0; i < count; i++) {
        fragment.appendChild(structureLiElementString("这里显示文章摘要，让读者对文章内容有个粗略的概念这里显示文章摘要，让读者对文章内容有个粗略的概念,这里显示文章摘要，让读者对文章内容有个粗略的概念"));
    }
    return fragment;
};
...
mui("#list").on('tap', 'div', function (event) {
    ...
    if(campusCircleClassList.contains('campusCircle-giveTheThumbsUp-list')){
        /**展示当前校园圈所有的点赞列表**/
        window.parent.controlSenseOfParticipationShow("giveTheThumbsUp");
    }else if(campusCircleClassList.contains('campusCircle-commentary-list')){
        /**展示当前校园圈所有的评论列表**/
...
mui("#list").on('tap', 'a', function (event) {
    ...
    var elementOfParent=obtainParentThroughClassName(this,document.getElementById("list"),"mui-card");
    var totalPraiseElement;
    var totalPraiseValue;
    if(elementOfParent=== undefined){

    }else{
        totalPraiseElement=elementOfParent.getElementsByClassName('campusCircle-total-number-praise')[0];
        if(totalPraiseElement=== undefined){
        ...
        if(praiseCampusCircles.length>0){
            ...
            if(praiseCampusCircleClassList.contains('icon-feedback-aixin')){
                ...
                if(totalPraiseElement=== undefined){
                    ...
                    totalPraiseElement.innerHTML=totalPraiseValue+1;
                }
